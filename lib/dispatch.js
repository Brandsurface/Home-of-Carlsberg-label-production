...
export async function dispatchToBrandsurface(order) {
  ...
  try {
    ...
    if (!hasMailKey() || !recipient) {
      console.warn('RESEND_API_KEY/SENDER_EMAIL eller modtager mangler — ingen mail sendt til Brandsurface')
      return
    }
    // Cancel the pending scheduled send so Resend doesn't also fire it later
